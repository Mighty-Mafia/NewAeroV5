...
run(function()
    ...
    AntiLag = vape.Categories.World:CreateModule({
        Name = 'Anti-Lag',
        Function = function(callback)
            ToggleState = callback
            if ToggleState then
                applyAntiLag()
                notif('Anti-Lag', 'Anti-Lag enabled!', 3)
            else
                revertAntiLag()
                notif('Anti-Lag', 'Anti-Lag disabled!', 3)
            end
        end,
        Tooltip = 'Reduces lag by lowering graphics and removing certain effects.'
    })
end)
...
run(function()
    ...
    local function findClosestBreakableBlock(start, playerPos)
        ...
        for _, side in sides do
            for i = 1, 15 do
                ...
                local block = getPlacedBlock and getPlacedBlock(blockPos)
    ...
    local function attemptBreak(tab, localPosition)
        if not tab or isBreaking then return false end
                
        local currentTime = tick()
        if currentTime - lastBreakTime < 0.1 then return false end
                
        for _, v in tab do
            if not v or not v.Position then continue end
                        
            local distance = (v.Position - localPosition).Magnitude
            if distance < Range.Value then
                local blockData = {blockPosition = v.Position / 3}
                if not bedwars.BlockController:isBlockBreakable(blockData, lplr) then continue end
                ...
                if LimitItem.Enabled then
                    if not store.hand.tool or not bedwars.ItemMeta[store.hand.tool.Name] or not bedwars.ItemMeta[store.hand.tool.Name].breakBlock then 
                        continue 
                    end
                end
                ...
                if BreakClosestBlock.Enabled and (v.Name == "bed" or collection('bed'):find(v)) then
                ...
                local success, target, path, endpos = pcall(function()
                    return bedwars.breakBlock(targetBlock, Effect.Enabled, Animation.Enabled, CustomHealth.Enabled and customHealthbar or nil, InstantBreak.Enabled)
                end)
                ...
                if success and path then
                    ...
                    for i, part in ipairs(parts) do
                        if part and part.Parent then
                            ...
                            if currentnode and part.BoxHandleAdornment then
                ...
                local waitTime = InstantBreak.Enabled and (store.damageBlockFail > tick() and 4.5 or 0.05) or BreakSpeed.Value
                task.wait(waitTime)
        ...
        Function = function(callback)
            if callback then
                ...
                customlist = collection('block', Breaker, function(tab, obj)
                    if Custom.ListEnabled and table.find(Custom.ListEnabled, obj.Name) then
                ...
                repeat
                    local success, err = pcall(function()
                        ...
                        if entitylib.isAlive and entitylib.character and entitylib.character.RootPart then
                            local localPosition = entitylib.character.RootPart.Position
                            if Bed.Enabled and attemptBreak(beds, localPosition) then
                                return
                            end
                            if attemptBreak(customlist, localPosition) then
                                return
                            end
                            if LuckyBlock.Enabled and attemptBreak(luckyblock, localPosition) then
                                return
                            end
                            if IronOre.Enabled and attemptBreak(ironores, localPosition) then
                                return
                            end
    ...
    Custom = Breaker:CreateTextList({
        ...
        Function = function()
            ...
            if store.blocks then
                for _, obj in ipairs(store.blocks) do
                    if Custom.ListEnabled and table.find(Custom.ListEnabled, obj.Name) then
    ...
    BreakClosestBlock = Breaker:CreateToggle({
        ...
        Default = false,
        Tooltip = 'Breaks the closest block when targeting beds, making it less blatant and more natural looking'
    })
end)
